...
const StorageManager = {
  ...
  serverSave: async function (jsonData) {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        console.warn('[Sync] Save warning:', result.error, result.detail || '');
      }
    } catch (err) {
  ...
  // ── Local load ────────────────────────────────────────────────────────────
  load: function () {
    return null; // Local caching removed. Forced to rely on MySQL.
  },
...
const InitialData = {
  ...
  // SARS 2026/2027 tax year (1 March 2026 – 28 February 2027)
  taxTables: [
    { year: '2026', min: 0, max: 245100, rate: 0.18, rebate: 0 },
    { year: '2026', min: 245101, max: 383100, rate: 0.26, rebate: 44118 },
    { year: '2026', min: 383101, max: 530200, rate: 0.31, rebate: 79998 },
    { year: '2026', min: 530201, max: 695800, rate: 0.36, rebate: 125599 },
    { year: '2026', min: 695801, max: 887000, rate: 0.39, rebate: 185215 },
    { year: '2026', min: 887001, max: 1878600, rate: 0.41, rebate: 259783 },
    { year: '2026', min: 1878601, max: 999999999, rate: 0.45, rebate: 666339 }
  ],
  settings: {
...
async function autoSync() {
  try {
    ...
    if (!serverData) {
      // No server data yet — push our initial state up (first run)
      const json = JSON.stringify(window.DB);
      StorageManager.serverSave(json);
      return;
    }

    // Server is the sole source of truth — merge into local runtime memory
    LIVE_KEYS.forEach(k => { if (serverData[k] !== undefined) window.DB[k] = serverData[k]; });
    console.log('[autoSync] Downloaded fresh MySQL database state ✓');
