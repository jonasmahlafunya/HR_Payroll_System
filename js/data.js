...
const StorageManager = {
  ...
  save: function (data) {
    try {
      ...
      // Strip base64 blobs to keep JSON small
      if (toSave.companies) {
...
const InitialData = {
  users: [
    { id: 1, username: 'admin', password: 'admin', role: 'Super Admin', name: 'Administrator', email: '', status: 'Active', lastLogin: '', permissions: ['all'] },
    { id: 2, username: 'payroll', password: 'payroll', role: 'Payroll Officer', name: 'Payroll Officer', email: '', status: 'Active', lastLogin: '', permissions: ['payroll.process', 'tax.view'] },
    { id: 3, username: 'hr', password: 'hr', role: 'HR Manager', name: 'HR Manager', email: '', status: 'Active', lastLogin: '', permissions: ['employees.view', 'employees.edit', 'payroll.view', 'reports.view'] }
  ],
  roles: [
...
//
// BUG FIX: The old autoSync unconditionally overwrote all keys with server
// data. If a user saved a new company and the async server-write hadn't
// finished, the next page load would call autoSync, load the old server state,
// and wipe the new company from memory.
//
// Fix: compare _savedAt timestamps. Only overwrite when the server copy is
// genuinely more recent than what we have in localStorage.
// ─────────────────────────────────────────────────────────────────────────────
async function autoSync() {
  try {
    ...
    if (serverTs < localTs) {
      // Local is newer — the server hasn't received our latest save yet.
      // Push local state up to bring the server in sync.
