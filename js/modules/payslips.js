const Payslips = {
    ...
    document.getElementById('runSlipsContainer').innerHTML = `
      <div class="card" style="margin-top:24px;">
        <div class="card-header">
          ...
          <button class="btn btn-outline btn-sm" onclick="Payslips.downloadBatch('${runId}')">
            <i class="fas fa-file-pdf"></i> Download PDF Batch
          </button>
        </div>
        <div class="table-responsive">
          <table>
        ...
        <div class="modal-header"
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            ...
            <button class="btn btn-primary btn-xs" 
              onclick="window.open('api/payslip_pdf.php?employee_id=${emp.id}&run_id=${line.runId}&format=pdf', '_blank')">
  ...
  downloadBatch: function (runId) {
    const slips = (window.DB.payslips || []).filter(p => p.runId == runId);
    if (!slips.length) return Toast.show('No payslips found for this run', 'warning');
    
    const ids = slips.map(s => s.employeeId || s.id).join(',');
    window.open(`api/payslip_pdf.php?employee_id=${ids}&run_id=${runId}&format=pdf`, '_blank');
  },
