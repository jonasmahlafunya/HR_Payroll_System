...
const PayrollValidation = {
    validatePayrollRun: function (payrollData) {
        ...
        employees.forEach(emp => {
            ...
            // Missing salary information (Basic or Hourly Rate)
            if ((!emp.basicSalary || emp.basicSalary <= 0) && (!emp.ratePerHour || emp.ratePerHour <= 0)) {
                errors.push({
                    type: 'critical',
                    employee: `${emp.firstName} ${emp.lastName}`,
                    employeeId: emp.id,
                    message: 'Missing or invalid salary/rate',
                    field: 'salary',
                    action: 'Set basic salary or hourly rate'
                });
            }
