...
const Settings = {
            ...
            ${['#4f46e5','#0ea5e9','#10b981','#f59e0b','#ef4444','#8b5cf6','#06b6d4','#ec4899','#64748b']
              .map(hex=>`<button onclick="Settings._setColor('${hex}')"
                style="width:28px;height:28px;border-radius:50%;background:${hex};cursor:pointer;
                       border:3px solid ${(s.primaryColor||'#4f46e5')===hex?'#1e293b':'transparent'};"></button>`).join('')}
            <input type="color" id="customColor" value="${s.primaryColor||'#4f46e5'}"
  ...
  saveNotifications: function() {
    this._setCS(this._companyId,{notifications:{
  ...
  saveAppearance: function() {
    ...
    const cc=document.getElementById('customColor'); if(cc?.value) s.primaryColor=cc.value;
  ...
  saveNewUser: function() {
    ...
    const u={id:(window.DB.users||[]).reduce((m,u)=>Math.max(m,u.id||0),0)+1
  ...
  editUserModal: function(uid) {
    ...
    const html=`<div class="card" style="width:100%;max-width:400px;margin:auto;">
      ...
      <div class="card-body">
        ...
        <div class="form-group"><label class="form-label">New Password</label><input id="eu_pwd" type="password" class="form-control"></div>
  ...
  importData: function() {
    ...
    const r=new FileReader();
    r.onload=e=>{try{Object.assign(window.DB,JSON.parse(e.target.result));window.DB.save();
