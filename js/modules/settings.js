...
const Settings = {
      ...
      <div style="display:grid;grid-template-columns:210px 1fr;gap:16px;align-items:start;">
        <div>
          ...
          <div class="card" style="padding:8px;">
            ...
            ${['users', 'permissions', 'appearance', 'security', 'backup', 'system'].map(k => Settings._navItem(k)).join('')}
          </div>
        </div>
  ...
  _labels: {
    ...
    notifications: 'Notifications', users: 'Users & Roles', permissions: 'Permissions',
    appearance: 'Appearance', security: 'Security', backup: 'Backup & Restore', system: 'System Info'
  },
  _icons: {
    company: 'fas fa-building', payroll: 'fas fa-money-bill-wave', leave: 'fas fa-calendar-alt',
    notifications: 'fas fa-bell', users: 'fas fa-users-cog', permissions: 'fas fa-user-shield',
    appearance: 'fas fa-palette', security: 'fas fa-shield-alt', backup: 'fas fa-database',
    system: 'fas fa-cogs'
  },
  ...
  addUserModal: function () {
    ...
    const html = `<div class="card" style="width:100%;max-width:500px;margin:auto;">
      <div class="card-header">
        <h3 class="card-title"><i class="fas fa-user-plus" style="color:var(--primary);margin-right:8px;"></i>Add New User</h3>
        <button class="btn btn-outline btn-sm" onclick="closeModal('addUserModal')"><i class="fas fa-times"></i></button>
      </div>
      <div class="card-body">
        <div style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25);border-radius:10px;padding:10px 14px;margin-bottom:18px;font-size:0.82rem;color:#92400E;display:flex;gap:8px;align-items:flex-start;">
          <i class="fas fa-envelope" style="margin-top:2px;"></i>
          <span><strong>Email is required for 2FA.</strong> The user will receive a one-time code at this address every time they log in.</span>
        </div>
        <div class="grid-2">
          <div class="form-group">
            <label class="form-label">Full Name</label>
            <input id="nu_name" class="form-control" placeholder="e.g. Thabo Dlamini">
          </div>
          <div class="form-group">
            <label class="form-label">Email Address <span style="color:var(--danger);">*</span></label>
            <input id="nu_email" type="email" class="form-control" placeholder="user@company.co.za" required>
          </div>
          <div class="form-group">
            <label class="form-label">Username <span style="color:var(--danger);">*</span></label>
            <input id="nu_user" class="form-control" placeholder="e.g. thabo.dlamini" autocomplete="off">
          </div>
          <div class="form-group">
            <label class="form-label">Password <span style="color:var(--danger);">*</span></label>
            <input id="nu_pwd" type="password" class="form-control" placeholder="Min. 8 characters" autocomplete="new-password">
          </div>
          <div class="form-group">
            <label class="form-label">Role</label>
            <select id="nu_role" class="form-control">${roles.map(r => `<option>${r.name}</option>`).join('')}</select>
          </div>
        </div>
        ${companies.length > 1 ? `<div class="form-group"><label class="form-label">Company Access</label>
          ...
          </select><div class="form-hint">Leave blank = access to all companies</div></div>` : ''}
        <button class="btn btn-primary" style="width:100%;margin-top:4px;" onclick="Settings.saveNewUser()">
          <i class="fas fa-save"></i> Create User &amp; Save to Database
        </button>
      </div>
    </div>`;
  ...
  saveNewUser: async function () {
    const un = document.getElementById('nu_user')?.value?.trim();
    const pw = document.getElementById('nu_pwd')?.value;
    const name = document.getElementById('nu_name')?.value?.trim() || un;
    const email = document.getElementById('nu_email')?.value?.trim() || '';
    const role = document.getElementById('nu_role')?.value || 'Employee';
    const sel = document.getElementById('nu_comps');
    const cids = sel ? [...sel.selectedOptions].map(o => parseInt(o.value)) : [];

    if (!un || !pw) { window.Toast.show('Username & password are required', 'warning'); return; }
    if ((window.DB.users || []).some(u => u.username === un)) {
      window.Toast.show('Username already taken', 'warning'); return;
    }
    ...
    try {
      // 1. Persist to MySQL database via API
      const res = await fetch('api/auth.php?action=create_user', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-Token': window.Auth?._csrfToken || '',
          'X-Auth-Token': document.cookie.match(/nexa_session=([^;]+)/)?.[1] || ''
        },
        body: JSON.stringify({ name, email, username: un, password: pw, role })
      });
      const data = await res.json();

      if (!res.ok) {
        window.Toast.show(data.error || 'Server error creating user', 'danger');
        return;
      }

      // 2. Mirror in local DB (keeps offline mode & permissions in sync)
      const newUser = {
        id: data.id || ((window.DB.users || []).reduce((m, u) => Math.max(m, u.id || 0), 0) + 1),
        name, email, username: un,
        role, companyIds: cids,
        status: 'Active', lastLogin: '', permissions: []
      };
      window.DB.users = window.DB.users || [];
      window.DB.users.push(newUser);
      window.DB.save();

      window.Toast.show(`User "${un}" created and saved to database`, 'success');
      window.closeModal('addUserModal');
      this._tab = 'users';
      this.render(document.getElementById('content'));

    } catch (err) {
      // Network error — still save locally so admin isn't blocked
      if (err.message.includes('Failed to fetch') || err.message.includes('NetworkError')) {
        const newUser = {
          id: (window.DB.users || []).reduce((m, u) => Math.max(m, u.id || 0), 0) + 1,
          name, email, username: un, password: pw,
          role, companyIds: cids,
          status: 'Active', lastLogin: '', permissions: []
        };
        window.DB.users.push(newUser);
        window.DB.save();
        window.Toast.show(`User "${un}" created locally (offline — will sync when reconnected)`, 'warning');
        window.closeModal('addUserModal');
        this._tab = 'users';
        this.render(document.getElementById('content'));
      } else {
        window.Toast.show('Failed to create user: ' + err.message, 'danger');
      }
    } finally {
  ...
  // ─── Permissions Tab ─────────────────────────────────────────────────────
  tabPermissions: function () {
    const roles = window.DB.roles || [];
    const allPerms = [
      { key: 'employees.view', label: 'View Employees', icon: 'fas fa-eye' },
      { key: 'employees.edit', label: 'Edit Employees', icon: 'fas fa-user-edit' },
      { key: 'payroll.view', label: 'View Payroll', icon: 'fas fa-file-invoice' },
      { key: 'payroll.process', label: 'Process Payroll', icon: 'fas fa-calculator' },
      { key: 'tax.view', label: 'Tax Compliance', icon: 'fas fa-receipt' },
      { key: 'reports.view', label: 'View Reports', icon: 'fas fa-chart-bar' },
      { key: 'leave.manage', label: 'Manage Leave', icon: 'fas fa-calendar-check' },
      { key: 'documents.manage', label: 'Manage Documents', icon: 'fas fa-folder' },
      { key: 'settings.admin', label: 'Admin Settings', icon: 'fas fa-cogs' },
      { key: 'self.view', label: 'Self-Service', icon: 'fas fa-user' }
    ];

    return `
      <div class="card">
        <div class="card-header">
          <h3 class="card-title"><i class="fas fa-user-shield text-primary"></i> Role Permission Matrix</h3>
          <button class="btn btn-primary btn-sm" onclick="Settings.savePermissions()">
            <i class="fas fa-save"></i> Save
          </button>
        </div>
          ...
          <table style="width:100%;font-size:0.8rem;">
            <thead>
              <tr style="background:var(--gray-50);">
                <th style="padding:10px 14px;font-weight:600;position:sticky;left:0;background:var(--gray-50);z-index:1;">Permission</th>
                ${roles.map(r => `
                  <th style="padding:10px 8px;text-align:center;min-width:100px;">
                    <div style="font-weight:700;color:var(--gray-800);">${r.name}</div>
                    <div style="font-size:0.65rem;color:var(--gray-400);font-weight:400;">${r.description || ''}</div>
                  </th>
                `).join('')}
              </tr>
            </thead>
            <tbody>
              ${allPerms.map(p => `
                <tr style="border-bottom:1px solid var(--gray-100);">
                  <td style="padding:8px 14px;position:sticky;left:0;background:white;z-index:1;">
                    <div style="display:flex;align-items:center;gap:6px;">
                      <i class="${p.icon}" style="color:var(--primary);width:14px;text-align:center;font-size:0.7rem;"></i>
                      <span style="font-weight:500;">${p.label}</span>
                    </div>
                    <div style="font-size:0.65rem;color:var(--gray-400);margin-left:20px;">${p.key}</div>
                  </td>
                  ${roles.map(r => {
      const hasAll = (r.permissions || []).includes('all');
      const hasPerm = hasAll || (r.permissions || []).includes(p.key);
      const isSuper = r.name === 'Super Admin';
      return `
                      <td style="text-align:center;padding:6px;">
                        <label style="cursor:${isSuper ? 'not-allowed' : 'pointer'};display:inline-flex;align-items:center;justify-content:center;">
                          <input type="checkbox" data-role="${r.id}" data-perm="${p.key}"
                            ${hasPerm ? 'checked' : ''} ${isSuper ? 'disabled' : ''}
                            style="width:18px;height:18px;accent-color:var(--primary);cursor:inherit;">
                        </label>
                      </td>`;
    }).join('')}
                </tr>
        ...
        <div style="padding:12px 16px;border-top:1px solid var(--gray-200);display:flex;justify-content:space-between;align-items:center;">
          <div style="font-size:0.72rem;color:var(--gray-400);">
            <i class="fas fa-info-circle"></i> Super Admin always has full access and cannot be modified.
          </div>
          <button class="btn btn-outline btn-sm" onclick="Settings.resetPermissions()">
  ...
  savePermissions: function () {
    const roles = window.DB.roles || [];
    const checkboxes = document.querySelectorAll('input[data-role][data-perm]');
    const rolePerms = {};

    checkboxes.forEach(cb => {
      const roleId = cb.dataset.role;
      const perm = cb.dataset.perm;
      if (!rolePerms[roleId]) rolePerms[roleId] = [];
      if (cb.checked) rolePerms[roleId].push(perm);
    });

    roles.forEach(r => {
      if (r.name === 'Super Admin') { r.permissions = ['all']; return; }
      r.permissions = rolePerms[r.id] || [];
    });

    // Also update users' permissions to match their role
    (window.DB.users || []).forEach(u => {
      const role = roles.find(r => r.name === u.role);
      if (role) u.permissions = [...role.permissions];
    });

    window.DB.save();
    window.Toast.show('Permissions saved successfully!', 'success');
  },

  resetPermissions: function () {
    window.showConfirmation('Reset Permissions', 'Reset all permissions to system defaults?', () => {
      window.DB.roles = [
        { id: 1, name: 'Super Admin', description: 'Full system access', permissions: ['all'] },
        { id: 2, name: 'HR Manager', description: 'Employee and payroll data', permissions: ['employees.view', 'employees.edit', 'payroll.view', 'reports.view', 'leave.manage', 'documents.manage'] },
        { id: 3, name: 'Payroll Officer', description: 'Process payroll and tax', permissions: ['payroll.process', 'payroll.view', 'tax.view', 'reports.view'] },
        { id: 4, name: 'Employee', description: 'Self-service access only', permissions: ['self.view'] }
      ];
      window.DB.save();
      window.Toast.show('Permissions reset to defaults', 'success');
      this.render(document.getElementById('content'));
    });
  },
  ...
  tabBackup: function () {
    return `
      <div class="grid-2" style="gap:16px;">
        ...
        <div class="card">
          <div class="card-header">
            <h3 class="card-title"><i class="fas fa-download text-primary"></i> Local Backup</h3>
          </div>
          <div class="card-body">
            <p style="font-size:0.82rem;color:var(--gray-600);margin-bottom:16px;">
              Download the entire system state as a JSON file. This includes all employees, payroll runs, documents, and settings.
            </p>
            <div style="background:var(--gray-50);border-radius:8px;padding:12px;margin-bottom:16px;">
              <div style="display:flex;justify-content:space-between;font-size:0.78rem;margin-bottom:4px;">
                <span style="color:var(--gray-500);">Records</span>
                <span style="font-weight:600;">${(window.DB.employees || []).length} employees, ${(window.DB.payrollRuns || []).length} runs</span>
              </div>
              <div style="display:flex;justify-content:space-between;font-size:0.78rem;">
                <span style="color:var(--gray-500);">Last saved</span>
                <span style="font-weight:600;">${window.DB._savedAt ? new Date(window.DB._savedAt).toLocaleString() : 'Never'}</span>
              </div>
            </div>
            <button class="btn btn-primary" style="width:100%;" onclick="Settings.downloadBackup()">
        ...
        <div class="card">
          <div class="card-header">
            <h3 class="card-title"><i class="fas fa-upload text-warning"></i> Restore from Backup</h3>
          </div>
          <div class="card-body">
            <p style="font-size:0.82rem;color:var(--gray-600);margin-bottom:16px;">
              Upload a previously downloaded <code>.json</code> backup file to restore the system. <strong style="color:var(--danger);">This will overwrite all current data.</strong>
            </p>
            <div id="dropZoneBackup"
              style="border:2px dashed var(--gray-300);border-radius:8px;padding:24px;text-align:center;
                     cursor:pointer;margin-bottom:12px;transition:border-color 0.2s;"
              onclick="document.getElementById('restoreFile').click()"
              ondragover="event.preventDefault();this.style.borderColor='var(--primary)'"
              ondragleave="this.style.borderColor='var(--gray-300)'"
              ondrop="event.preventDefault();document.getElementById('restoreFile').files=event.dataTransfer.files;Settings.previewRestore()">
              <i class="fas fa-cloud-upload-alt" style="font-size:1.5rem;color:var(--gray-400);margin-bottom:6px;display:block;"></i>
              <div style="font-size:0.82rem;color:var(--gray-500);">Click or drag a .json backup file</div>
            </div>
            <input type="file" id="restoreFile" accept=".json" style="display:none;" onchange="Settings.previewRestore()">
      ...
      <div class="card" style="margin-top:16px;">
        <div class="card-header">
          <h3 class="card-title"><i class="fas fa-server text-info"></i> Server Database Backup</h3>
          <button class="btn btn-sm btn-primary" onclick="Settings.triggerServerBackup()">
            <i class="fas fa-database"></i> Trigger Backup Now
          </button>
        </div>
        <div class="card-body">
          <p style="font-size:0.82rem;color:var(--gray-600);margin-bottom:16px;">
            Server-side MySQL backups are stored on the server and can be used for full database restoration.
          </p>
          <div id="backupList" style="color:var(--gray-400);font-size:0.82rem;">
            <i class="fas fa-spinner fa-spin"></i> Loading backup history...
          </div>
        </div>
      </div>
      <script>Settings.loadBackupHistory();</script>
    `;
  },

  downloadBackup: function () {
    const data = JSON.parse(JSON.stringify(window.DB));
    delete data.save; // Remove function reference
    data._backupDate = new Date().toISOString();
    data._backupVersion = '2.0';
    ...
    a.href = url;
    a.download = 'nexa_backup_' + ts + '.json';
    a.click();
    URL.revokeObjectURL(url);
    window.Toast.show('Backup downloaded!', 'success');
  },

  previewRestore: function () {
    ...
    reader.onload = e => {
      try {
        ...
        preview.innerHTML = `
          <div style="background:var(--warning-soft);border:1px solid var(--warning);border-radius:8px;padding:12px;margin-bottom:12px;">
            <div style="font-size:0.78rem;font-weight:700;color:var(--warning);margin-bottom:6px;">
              <i class="fas fa-exclamation-triangle"></i> Restore Preview
            </div>
            <div style="font-size:0.75rem;color:var(--gray-700);line-height:1.6;">
              ${Object.entries(counts).map(([k, v]) => `<strong>${v}</strong> ${k}`).join(' • ')}
            </div>
            <div style="font-size:0.7rem;color:var(--gray-500);margin-top:4px;">
              Backup date: ${data._backupDate ? new Date(data._backupDate).toLocaleString() : 'Unknown'}
            </div>
          </div>
          <button class="btn btn-sm" style="width:100%;background:var(--danger);color:white;" onclick="Settings.executeRestore()">
            <i class="fas fa-exclamation-circle"></i> Confirm Restore — Overwrite Current Data
          </button>
        `;
        window._pendingRestore = data;
      } catch (err) {
        preview.innerHTML = '<div class="alert alert-danger"><i class="fas fa-times-circle"></i> Invalid JSON file.</div>';
      }
    };
  ...
  executeRestore: function () {
    if (!window._pendingRestore) { window.Toast.show('No backup loaded', 'warning'); return; }
    window.showConfirmation('Confirm Restore',
      '<strong style="color:var(--danger)">This will overwrite ALL current data with the backup. This cannot be undone.</strong>',
      () => {
        const data = window._pendingRestore;
        const LIVE_KEYS = [
          'employees', 'companies', 'departments', 'payrollRuns', 'payslips', 'leaveRequests',
          'auditLogs', 'contracts', 'benefits', 'shifts', 'timesheets', 'overtimeRequests',
          'surveys', 'successionPlans', 'trainingRecords', 'disciplinaryCases', 'notifications',
          'documents', 'payrollCalendars', 'clientInvoices', 'recruitment', 'onboarding', 'offboarding',
          'performance', 'training', 'settings', 'users', 'roles', 'workflowRules', 'billingRates'
        ];
        LIVE_KEYS.forEach(k => { if (data[k] !== undefined) window.DB[k] = data[k]; });
        window.DB.save();
        delete window._pendingRestore;
        window.Toast.show('System restored from backup!', 'success');
        setTimeout(() => window.location.reload(), 1500);
      }
    );
  },

  triggerServerBackup: async function () {
    window.Toast.show('Triggering server backup...', 'info');
    try {
      const res = await fetch('api/backup.php?type=auto', {
        credentials: 'include',
        headers: { 'X-Auth-Token': document.cookie.match(/nexa_session=([^;]+)/)?.[1] || '' }
      });
      const data = await res.json();
      if (data.status === 'success') {
        window.Toast.show('Server backup created: ' + data.filename, 'success');
        this.loadBackupHistory();
      } else {
        window.Toast.show(data.error || 'Backup failed', 'danger');
      }
    } catch (err) {
      window.Toast.show('Server backup failed: ' + err.message, 'danger');
    }
  },
  ...
  loadBackupHistory: async function () {
    ...
    try {
      ...
      if (data.backups && data.backups.length) {
        ...
        container.innerHTML = '<div style="padding:12px;text-align:center;color:var(--gray-400);font-size:0.82rem;">No server backups found.</div>';
      }
    } catch (err) {
      container.innerHTML = '<div style="color:var(--gray-400);font-size:0.82rem;"><i class="fas fa-exclamation-triangle"></i> Could not load backup history (server may be offline).</div>';
    }
  }
};
