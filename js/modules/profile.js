...
const Profile = {
  ...
  editProfile: function () {
    ...
    window._profileEditData = {}; // Initialize cache
    this._switchEditTab('personal');
  },

  _flushProfileEditTab: function () {
    const data = window._profileEditData;
    if (!data) return;
    const g = id => { const el = document.getElementById(id); return el ? el.value.trim() : undefined; };
    // Personal
    if (g('ep_firstName') !== undefined) data.firstName = g('ep_firstName');
    if (g('ep_lastName') !== undefined) data.lastName = g('ep_lastName');
    if (g('ep_email') !== undefined) data.email = g('ep_email');
    if (g('ep_phone') !== undefined) data.phone = g('ep_phone');
    if (g('ep_idNumber') !== undefined) data.idNumber = g('ep_idNumber');
    if (g('ep_dob') !== undefined) data.dateOfBirth = g('ep_dob');
    if (g('ep_gender') !== undefined) data.gender = g('ep_gender');
    if (g('ep_nationality') !== undefined) data.nationality = g('ep_nationality');
    if (g('ep_address') !== undefined) data.address = g('ep_address');
    // Employment
    ...
    if (g('ep_dept') !== undefined) data.department = g('ep_dept');
    if (g('ep_position') !== undefined) data.position = g('ep_position');
    if (g('ep_empNo') !== undefined) data.employeeNumber = g('ep_empNo');
    if (g('ep_startDate') !== undefined) data.startDate = g('ep_startDate');
    if (g('ep_empType') !== undefined) data.employmentType = g('ep_empType');
    const salEl = document.getElementById('ep_salary');
    if (salEl) { const s = parseFloat(salEl.value); if (!isNaN(s)) data.basicSalary = s; }
    if (g('ep_status') !== undefined) data.status = g('ep_status');
    if (g('ep_taxNo') !== undefined) data.taxNumber = g('ep_taxNo');
    if (g('ep_uif') !== undefined) data.uifNumber = g('ep_uif');
    // Banking
    if (g('ep_bank') !== undefined) data.bankName = g('ep_bank');
    if (g('ep_accNo') !== undefined) data.accountNumber = g('ep_accNo');
    if (g('ep_branchCode') !== undefined) data.branchCode = g('ep_branchCode');
    if (g('ep_accType') !== undefined) data.accountType = g('ep_accType');
    // Account
    if (g('ep_displayName') !== undefined) data.displayName = g('ep_displayName');
    if (g('ep_loginEmail') !== undefined) data.loginEmail = g('ep_loginEmail');
    if (g('ep_newPass') !== undefined) data.newPass = g('ep_newPass');
    if (g('ep_confirmPass') !== undefined) data.confirmPass = g('ep_confirmPass');
  },

  _switchEditTab: function (tab) {
    this._flushProfileEditTab(); // Persist current tab values before switching
  ...
  saveProfile: function () {
    this._flushProfileEditTab(); // Capture any remaining visible fields
    ...
    // ── Personal + Employment + Banking fields (from cache) ──────────────────
    if (emp) {
      const fn = (data.firstName || '').trim();
      const ln = (data.lastName  || '').trim();
      if (fn) { emp.firstName = fn; savedAnything = true; }
      if (ln) { emp.lastName  = ln; savedAnything = true; }
      ...
      directFields.forEach(([prop, val]) => {
        if (val !== undefined && val !== '') { emp[prop] = val; savedAnything = true; }
      });

      if (data.basicSalary !== undefined && data.basicSalary >= 0) {
        emp.basicSalary = data.basicSalary; savedAnything = true;
      }
    ...
    let confirmPas  = data.confirmPass || '';
    let displayName = data.displayName || '';
    let loginEmail  = data.loginEmail  || '';

    // If Admin (no emp record), map Personal tab inputs if they were flushed
    if (!emp && cu) {
    ...
    if (cu) {
      if (displayName && displayName !== cu.name) { cu.name = displayName; savedAnything = true; }
      if (loginEmail && loginEmail !== cu.email) { cu.email = loginEmail; savedAnything = true; }

      const userRec = (window.DB.users||[]).find(u =>
        u.username === cu.username || u.id === cu.id
      );
      if (userRec) {
        if (displayName && displayName !== userRec.name) { userRec.name = displayName; savedAnything = true; }
        if (loginEmail && loginEmail !== userRec.email) { userRec.email = loginEmail; savedAnything = true; }
      }

      if (newPass) {
        if (userRec) userRec.password = newPass;
        savedAnything = true;
        // MUST hit the actual backend endpoint since sync.php ignores password modifications
        fetch('api/auth.php?action=change_password', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': window.Auth?._csrfToken||'' },
          body: JSON.stringify({ username: cu.username, password: newPass })
        }).catch(err => console.warn('Failed to submit password change:', err));
      }
