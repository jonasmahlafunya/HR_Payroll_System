...
const Profile = {
  render: function (container) {
    ...
    // Clear the ID after use so navigating back to profile shows self by default
    ...
    const html = `
        <div class="card" style="border: none; background: transparent; box-shadow: none; margin-bottom: 8px;">
          <div style="display: flex; align-items: center; gap: 20px;">
            ...
            <div>
               <button class="btn btn-outline btn-sm" onclick="Profile.editProfile()">Edit Profile</button>
               <button class="btn btn-primary btn-sm" onclick="document.getElementById('profilePhotoInput').click()">Change Photo</button>
               <input type="file" id="profilePhotoInput" style="display:none" accept="image/*" onchange="Profile.handlePhotoUpload(this)">
            </div>
          </div>
        </div>
  
        <div class="tabs" style="margin-bottom: 24px;">
          <button class="tab-btn active" onclick="Profile.switchTab('personal', event)">Details</button>
          <button class="tab-btn" onclick="Profile.switchTab('banking', event)">Banking & Tax</button>
          <button class="tab-btn" onclick="Profile.switchTab('payslips', event)">Payslips</button>
          <button class="tab-btn" onclick="Profile.switchTab('documents', event)">Documents</button>
        </div>
  ...
  renderPersonalTab: function (emp) {
    return `
        ...
        <div class="card" style="margin-top: 16px;">
          <div class="card-header"><h4 class="card-title"><i class="fas fa-shield-alt text-primary"></i> Data Privacy & POPIA</h4></div>
          <div class="card-body" style="font-size: 0.85rem; color: var(--gray-600);">
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--gray-200); padding-bottom: 8px; margin-bottom: 8px;">
              <div><strong>Consent Flag:</strong> Granted for employment & payroll processing purposes</div>
              <span class="badge badge-success">Active</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--gray-200); padding-bottom: 8px; margin-bottom: 8px;">
              <div><strong>Retention Policy:</strong> 5 Years post-termination (LRA/BCEA)</div>
              <span>Standard</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 4px;">
              <div><strong>Right to Erasure:</strong> Employee data will be obfuscated after retention period expires.</div>
              <button class="btn btn-outline btn-sm text-danger" onclick="window.showAlert('Action Restricted', 'Data cannot be erased while employment is active and within statutory retention periods.')"><i class="fas fa-eraser"></i> Request Deletion</button>
            </div>
          </div>
        </div>
      `;
  ...
  renderPayslipsTab: function (emp) {
    // payslips store employeeId (number), not the string composite id
    const slips = window.DB.payslips ? window.DB.payslips.filter(p => p.employeeId === emp.id) : [];
    ...
    return `
      ...
      const date = run ? new Date(run.finalizedDate || run.createdDate).toLocaleDateString('en-ZA') : '-';
      return `
                           <tr>
                              <td>${date}</td>
                              <td>${period}</td>
                              ...
                              <td><button class="btn btn-sm btn-outline" onclick="Payslips.renderPayslipModal(${s.runId}, ${s.employeeId})">View</button></td>
                           </tr>
                        `;
    }).join('')}
                  </tbody>
  ...
  editProfile: function (employeeId = null) {
    ...
    if (!emp && user) {
      // Mock employee for admin user who doesn't have an employee record
      emp = { 
        isUserOnly: true, 
        id: 'user_' + user.id, 
  ...
  getEditPersonalHtml: function (emp) {
    // Safely destructure or ensure default empty string
    return `
            <div class="grid-2">
                ...
                <div class="form-group"><label class="form-label">Gender</label><select id="edit_gen" class="form-control"><option value="Male" ${emp.gender==='Male'?'selected':''}>Male</option><option value="Female" ${emp.gender==='Female'?'selected':''}>Female</option></select></div>
  ...
  getEditBankingHtml: function (emp) {
    return `
             <div class="grid-2">
                ...
                <div class="form-group"><label class="form-label">Acc Type</label><select id="edit_type" class="form-control"><option value="Savings" ${emp.accountType==='Savings'?'selected':''}>Savings</option><option value="Current" ${emp.accountType==='Current'?'selected':''}>Current</option></select></div>
  ...
  saveProfile: function (id) {
    ...
    // Attempt to match from database by ID if a valid db employee ID
    ...
    if (emp.isUserOnly && window.currentUser) {
       // Mock employee corresponding to the current admin user
       window.currentUser.name = `${emp.firstName} ${emp.lastName}`.trim();
       window.currentUser.email = emp.email;
       // We can also sync this back to DB.users
       const dbUser = window.DB.users.find(u => u.id === window.currentUser.id);
       if (dbUser) {
           dbUser.name = window.currentUser.name;
           dbUser.email = window.currentUser.email;
       }
       window.DB.save();
    } else {
    ...
    // Ensure profile reflects the update right away locally
    window.currentEmployee = emp;
    ...
    if (input.files && input.files[0]) {
      ...
      reader.onload = function (e) {
        ...
        if (emp) {
            ...
            window.Toast.show("Photo updated successfully", "success");
        }
      };
