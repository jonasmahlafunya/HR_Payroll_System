...
const Employees = {
  ...
  _flushEditTab: function () {
    ...
    if (g('edit_payFreq') !== undefined) data.payFrequency = g('edit_payFreq');
    // Banking
      ...
      content.innerHTML = `
        ...
        `<option value="${t}" ${emp.employmentType === t ? 'selected' : ''}>${t}</option>`).join('')}
          ...
          <div class="form-group"><label class="form-label">Pay Frequency</label>
            <select id="edit_payFreq" class="form-control">
              <option value="Monthly" ${(emp.payFrequency || 'Monthly') === 'Monthly' ? 'selected' : ''}>Monthly</option>
              <option value="Bi-Weekly" ${emp.payFrequency === 'Bi-Weekly' ? 'selected' : ''}>Bi-Weekly</option>
              <option value="Weekly" ${emp.payFrequency === 'Weekly' ? 'selected' : ''}>Weekly</option>
            </select>
            <div class="form-hint">Default: follows company setting</div></div>
  ...
  saveEmployee: function (empId) {
    ...
    if (data.payFrequency) emp.payFrequency = data.payFrequency;
        ...
        <div class="card-body">
          ...
          <div id="addEmpPane_employment" style="display:none;">
            <div class="grid-2">
              ...
              <div class="form-group"><label class="form-label">Pay Frequency</label>
                <select id="emp_payFreq" class="form-control">
  ...
  createEmployee: function (_unused) {
    try {
      ...
      const emp = {
        ...
        payFrequency: g('emp_payFreq') || 'Monthly',
        bankName: g('emp_bank'),
