...
const Employees = {
      ...
      content.innerHTML = `
        <div class="grid-2">
          <div class="form-group"><label class="form-label">Company</label>
            <select id="edit_company" class="form-control">
              <option value="">— Select —</option>
              ${companies.map(c=>`<option value="${c.name}" data-id="${c.id}" ${emp.companyName===c.name?'selected':''}>${c.name}</option>`).join('')}
            </select></div>
          <div class="form-group"><label class="form-label">Department</label>
            <input id="edit_department" class="form-control" value="${emp.department||''}"
              list="deptList_edit">
            <datalist id="deptList_edit">
          ...
          <div class="form-group"><label class="form-label">Basic Salary (R)</label>
            <input id="edit_salary" type="number" class="form-control" value="${emp.basicSalary||0}"></div>
  ...
  saveEmployee: function (empId) {
    ...
    // Personal
    const firstName = g('edit_firstName');
    const lastName  = g('edit_lastName');
    ...
    emp.email              = g('edit_email')     || emp.email;
    emp.phone              = g('edit_phone')     || emp.phone;
    emp.cellphone          = emp.phone;
    emp.idNumber           = g('edit_idNumber')  || emp.idNumber;
    emp.dateOfBirth        = g('edit_dob')       || emp.dateOfBirth;
    emp.gender             = g('edit_gender')    || emp.gender;
    emp.nationality        = g('edit_nationality')|| emp.nationality;
    emp.address            = g('edit_address')   || emp.address;
    ...
    emp.department       = g('edit_department')  || emp.department;
    emp.position         = g('edit_position')    || emp.position;
    emp.employeeNumber   = g('edit_empNo')       || emp.employeeNumber;
    emp.startDate        = g('edit_startDate')   || emp.startDate;
    emp.employmentType   = g('edit_empType')     || emp.employmentType;
    const newSalary = parseFloat(document.getElementById('edit_salary')?.value || 0);
    if (!isNaN(newSalary) && newSalary >= 0) emp.basicSalary = newSalary;
    emp.status           = g('edit_status')      || emp.status;
    emp.taxNumber        = g('edit_taxNo')       || emp.taxNumber;
    emp.uifNumber        = g('edit_uif')         || emp.uifNumber;

    // Banking
    emp.bankName         = g('edit_bank')        || emp.bankName;
    emp.accountNumber    = g('edit_accNo')       || emp.accountNumber;
    emp.branchCode       = g('edit_branchCode')  || emp.branchCode;
    emp.accountType      = g('edit_accType')     || emp.accountType;
          ...
          <div id="addEmpPane_employment" style="display:none;">
            <div class="grid-2">
              <div class="form-group"><label class="form-label">Company <span style="color:var(--danger)">*</span></label>
                <select id="emp_company" class="form-control">
                  <option value="">— Select Company —</option>
                  ${companies.map(co=>`<option value="${co.name}" data-id="${co.id}">${co.name}</option>`).join('')}
                </select>
                ${!companies.length ? '<div class="form-hint" style="color:var(--danger);">No companies yet — <a href="#" onclick="loadPage('companies');closeModal('addEmpModal')">add a company first</a></div>' : ''}
              </div>
              <div class="form-group"><label class="form-label">Department</label>
  ...
  createEmployee: function (_unused) {
    try {
      ...
      const emp = {
        ...
        basicSalary:     parseFloat(document.getElementById('emp_salary')?.value) || 0,
