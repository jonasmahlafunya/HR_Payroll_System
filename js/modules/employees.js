const Employees = {
  ...
  createEmployeeFromWizard: function (data) {
    try {
      ...
      window.Toast.show(`Employee ${newEmp.firstName} ${newEmp.lastName} created successfully!`, 'success');
      window.closeModal('addEmployeeWizard');
      delete window.currentWizard;
      this.render(document.getElementById('content'));
    } catch (err) {
      console.error('[Employees] createEmployeeFromWizard error:', err);
      window.Toast.show('Error creating employee: ' + err.message, 'warning');
    }
  },
