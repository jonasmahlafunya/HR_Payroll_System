...
const Benefits = {
            ...
            <tbody>
              ${benefits.map((b, i) => `
                <tr>
                  ...
                  <td><span class="badge badge-info">${b.type || 'Allowance'}</span></td>
                  <td>${b.calc || 'Fixed'}</td>
                  <td>${(b.calc || 'Fixed') === 'Fixed' ? `R ${(Number(b.val) || 0).toFixed(2)}` : `${(Number(b.val) || 0)}%`}</td>
      ...
      <div class="card" style="width:100%;max-width:500px;margin:auto;">
        ...
        <div class="card-body">
          ...
          <div class="form-group" style="margin-top:12px;">
            <label class="form-check" style="margin-bottom:8px; display:flex; gap:8px;">
              <input type="checkbox" id="edit_ben_uif" ${b.includeInUIF !== false ? 'checked' : ''}> Include in UIF Calculation
            </label>
            <label class="form-check" style="margin-bottom:8px; display:flex; gap:8px;">
              <input type="checkbox" id="edit_ben_sdl" ${b.includeInSDL !== false ? 'checked' : ''}> Include in SDL Calculation
            </label>
            <label class="form-check" style="display:flex; gap:8px;">
              <input type="checkbox" id="edit_ben_coida" ${b.includeInCOIDA !== false ? 'checked' : ''}> Include in COIDA Calculation
            </label>
          </div>
  ...
  updateLibraryBenefit: function (index) {
    ...
    const includeInUIF = document.getElementById('edit_ben_uif').checked;
    const includeInSDL = document.getElementById('edit_ben_sdl').checked;
    const includeInCOIDA = document.getElementById('edit_ben_coida').checked;

    if (!name) { window.Toast.show("Name is required", "warning"); return; }

    window.DB.benefits[index] = { ...window.DB.benefits[index], name, type, calc, val, includeInUIF, includeInSDL, includeInCOIDA };
    window.DB.save();
    ...
    const html = `
      <div class="card" style="width:100%;max-width:500px;margin:auto;">
        ...
        <div class="card-body">
          ...
          <div class="form-group" style="margin-top:12px;">
            <label class="form-check" style="margin-bottom:8px; display:flex; gap:8px;">
              <input type="checkbox" id="ben_uif" checked> Include in UIF Calculation
            </label>
            <label class="form-check" style="margin-bottom:8px; display:flex; gap:8px;">
              <input type="checkbox" id="ben_sdl" checked> Include in SDL Calculation
            </label>
            <label class="form-check" style="display:flex; gap:8px;">
              <input type="checkbox" id="ben_coida" checked> Include in COIDA Calculation
            </label>
          </div>
  ...
  createBenefit: function () {
    ...
    const includeInUIF = document.getElementById('ben_uif').checked;
    const includeInSDL = document.getElementById('ben_sdl').checked;
    const includeInCOIDA = document.getElementById('ben_coida').checked;

    if (!name) { window.Toast.show("Name is required", "warning"); return; }
    window.DB.benefits = window.DB.benefits || [];
    window.DB.benefits.push({ name, type, calc, val, provider: 'Internal', icon: 'hand-holding-usd', color: 'warning', includeInUIF, includeInSDL, includeInCOIDA });
    window.DB.save();
