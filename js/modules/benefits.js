...
const Benefits = {
   ...
   calculatePreview: function (basicSalary) {
      ...
      // Ensure basicSalary is a valid number — the caller passes emp.basicSalary,
      // but the param may arrive as undefined if called without args; default to 0.
      ...
      // Find correct tier based on the passed salary; fall back to highest tier
      const tier = tiers.find(t => salary >= t.min && salary <= t.max) || tiers[tiers.length - 1];

      if (tier) {
         let total = tier.main; // Main member cost
         if (dependents > 0) {
            total += tier.adult; // First dependent as adult/spouse
            if (dependents > 1) {
