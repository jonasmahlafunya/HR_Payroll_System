...
const Payroll = {
  ...
  calculateDraft: function () {
    ...
    // Match employees by companyName (for wizard-added employees) OR by companyId, OR no company set (seed data)
    const employees = window.DB.employees.filter(e => {
      if (e.status !== 'Active') return false;
      // Already explicitly linked by companyId
      if (e.companyId && e.companyId == this.state.selectedCompany) return true;
      // Linked by name from the dropdown selection
      if (e.companyName && selectedCompObj && e.companyName === selectedCompObj.name) return true;
      // Seed data: no companyName → belong to first company (id == 1) by convention
      if (!e.companyName && this.state.selectedCompany == 1) return true;
      return false;
    });

    this.state.draftData = employees.map(emp => {
      // Guard missing dateOfBirth — default age 30 to avoid crashes
