...
const Payroll = {
  ...
  showCreateRunModal: function () {
    ...
    this._runCalcs = {};
    if (companyId) {
      setTimeout(() => this._onCompanyChange(companyId), 50);
    }
  },
  ...
  calculateTax: function (emp, hoursWorked = 0, extra = {}) {
    ...
    // ── UIF & SDL (via TaxCalc) ──────────────────────────────────────────
    const uif = window.TaxCalc ? window.TaxCalc.calculateUIF(uifRemuneration) : Math.min(uifRemuneration * 0.01, 177.12);
