...
const Payroll = {
    ...
    container.innerHTML = `
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <button class="btn btn-outline btn-sm" onclick="PayrollWizard.start()" id="btnRunWizard">
            <i class="fas fa-magic"></i> Run Payroll Wizard
          </button>
          <button class="btn btn-outline btn-sm" onclick="Payroll.showBulkImportModal()">
                ...
                <div style="display:flex;align-items:center;gap:12px;">
                  ...
                  <div>
                    ...
                    <div style="font-size:0.75rem;color:var(--gray-500);">
                      ...
                      ${r.employeeCount || 0} employees &bull; Run: ${r.runDate || r.createdDate?.substring(0, 10) || '—'}
                    </div>
                  </div>
                </div>
    ...
    return `
      <div class="table-responsive">
        <table>
          <thead>
            <tr>
              <th>Period</th><th>Run Date</th><th>Company</th><th>Frequency</th>
      ...
      return `
                <tr>
                  <td><div style="font-weight:700;">${r.period}</div></td>
                  <td><div style="font-size:0.7rem;color:var(--gray-400);">${r.runDate || r.createdDate?.substring(0, 10) || '—'}</div></td>
                  <td style="font-size:0.82rem;">${r.company || '—'}</td>
                  <td><span class="badge badge-info" style="font-size:0.6rem;">${r.payFrequency || r.frequency || 'Monthly'}</span></td>
                  <td style="text-align:center;">${r.employeeCount || 0}</td>
  ...
  showCreateRunModal: function () {
    ...
    this._runCalcs = {};
    // FIXED: 'companyId' was undefined here — removed the erroneous early call.
    // The company change is handled by the onchange event on the dropdown.
  },
  ...
  _renderEmployeePreview: function (employees, comp) {
    ...
    const isHourly = comp.payFrequency === 'Weekly' || comp.payFrequency === 'Bi-Weekly';

    const rows = employees.map(emp => {
      const defaultHours = isHourly ? 40 : 0;
      const c = this.calculateTax(emp, defaultHours);
      this._runCalcs[emp.id] = { emp, hoursWorked: defaultHours, ...c };
      totGross += c.gross;
      totPAYE += c.paye;
      totUIF += c.uif;
      totNet += c.net;
      ...
      return `
        <div style="display:grid;grid-template-columns:${isHourly ? '2fr 80px 1fr 1fr 1fr 1fr 70px' : '2fr 1fr 1fr 1fr 1fr 70px'};
          ...
          ${isHourly ? `<div style="padding-right:10px;"><input type="number" class="form-control run-emp-hours" style="width:100%;padding:4px;height:24px;font-size:0.8rem;" value="${defaultHours}" data-empid="${emp.id}" onchange="Payroll._updateEmpRow(${emp.id}, this)"></div>` : ''}
          <div class="col-gross" style="text-align:right;font-size:0.8rem;">${window.formatCurrency(c.gross)}</div>
          <div class="col-paye" style="text-align:right;font-size:0.8rem;color:var(--danger);">${window.formatCurrency(c.paye)}</div>
          <div class="col-uif" style="text-align:right;font-size:0.8rem;color:var(--warning);">${window.formatCurrency(c.uif)}</div>
          <div class="col-net" style="text-align:right;font-size:0.8rem;font-weight:700;color:var(--success);">${window.formatCurrency(c.net)}</div>
          <div style="text-align:center;">
    ...
    preview.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:${isHourly ? '2fr 80px 1fr 1fr 1fr 1fr 70px' : '2fr 1fr 1fr 1fr 1fr 70px'};
        ...
        ${isHourly ? '<div style="font-size:0.65rem;text-transform:uppercase;color:var(--gray-500);font-weight:700;">Hours</div>' : ''}
  ...
  _updateEmpRow: function (empId, inputEl) {
    const emp = (window.DB.employees || []).find(e => e.id == empId);
    if (!emp) return;
    const hoursWorked = parseFloat(inputEl.value) || 0;
    const c = this.calculateTax(emp, hoursWorked);

    this._runCalcs[empId] = { emp, hoursWorked, ...c };
    const rowEl = document.querySelector(`.preview-row[data-empid="${empId}"]`);
    if (rowEl) {
      rowEl.querySelector('.col-gross').innerText = window.formatCurrency(c.gross);
      rowEl.querySelector('.col-paye').innerText = window.formatCurrency(c.paye);
      rowEl.querySelector('.col-uif').innerText = window.formatCurrency(c.uif);
      rowEl.querySelector('.col-net').innerText = window.formatCurrency(c.net);
    }
    this._updateRunTotals();
  },
  ...
  createRun: function () {
    ...
    // Generate a cryptographically secure approval token (not btoa which is easily decoded)
    ...
    // Add each payslip to the Documents tab as well
    window.DB.documents = window.DB.documents || [];
    payslips.forEach(p => {
      const emp = (window.DB.employees || []).find(e => e.id == p.employeeId);
      if (!emp) return;
      window.DB.documents.push({
        id: 'DOC_' + runId + '_' + p.employeeId,
        name: `Payslip — ${emp.firstName} ${emp.lastName} (${periodLabel})`,
        category: 'Payslip',
        employeeId: emp.id,
        companyWide: false,
        description: `${periodLabel} payslip for ${emp.firstName} ${emp.lastName}`,
        size: 0,
        mimeType: 'application/json',
        fileName: `Payslip_${p.employeeId}_${periodLabel.replace(/\s/g, '_')}.pdf`,
        dataUrl: null,
  ...
  // 2026/2027 tax year
  // ─────────────────────────────────────────────────────────────────────────
  calculateTax: function (emp, hoursWorked = 0, extra = {}) {
    const compId = emp.companyId || emp._companyId;
    const comp = (window.DB.companies || []).find(c => c.id == compId || c.name === emp.companyName);
    const isWeekly = comp && comp.payFrequency === 'Weekly';
    const isBiWeekly = comp && comp.payFrequency === 'Bi-Weekly';
    const freqMult = isWeekly ? 52 : (isBiWeekly ? 26 : 12);

    let basic = 0;
    if (isWeekly || isBiWeekly) {
      if (emp.ratePerHour > 0) {
        basic = Number(emp.ratePerHour) * hoursWorked;
      } else if (emp.basicSalary > 0) {
        // Splitting monthly salary by weeks (Annual / freq)
        basic = (Number(emp.basicSalary) * 12) / freqMult;
      }
    } else {
    ...
    const otPay = Number(extra.otPay || emp.otPay || 0);
    const phPay = Number(extra.phPay || emp.phPay || 0);

    let medical = Number(emp.medicalAid || emp.medical || 0);
    let pension = Number(emp.pension || emp.pensionContribution || 0);
    let garnishee = Number(emp.garnishee || 0);

    // Custom benefits from employee record
    const legacyBenefits = (window.DB.benefits || []).filter(b => b.employeeId == emp.id && b.status === 'Active');
    const empCustom = emp.benefits?.custom || [];
    const joinedBenefits = [...legacyBenefits, ...empCustom];

    const customBenefits = joinedBenefits.map(b => ({
      name: b.name,
      type: b.type,
      calc: b.calculation || b.calc || 'Fixed',
      value: b.value,
      includeInUIF: b.includeInUIF !== false,
      includeInSDL: b.includeInSDL !== false,
      includeInCOIDA: b.includeInCOIDA !== false
    }));
    ...
    const gross = basic + bonus + otPay + phPay + allowanceSum;

    // Prorate monthly benefits for weekly/bi-weekly
    medical = (medical * 12) / freqMult;
    pension = (pension * 12) / freqMult;
    garnishee = (garnishee * 12) / freqMult;

    const uifRemuneration = basic + bonus + otPay + phPay + allowances.filter(b => b.includeInUIF).reduce((s, b) => s + calcBenVal(b), 0);
    const sdlRemuneration = basic + bonus + otPay + phPay + allowances.filter(b => b.includeInSDL).reduce((s, b) => s + calcBenVal(b), 0);

    // ── PAYE (SARS Brackets via TaxCalc) ──────────────────────────────────
    const age = window.TaxCalc ? window.TaxCalc.calculateAge(emp.dateOfBirth) : 35;
    const members = Number(emp.medicalMembers || 0);
    const taxableAnnual = (gross - pension) * freqMult;

    const payeAnnual = window.TaxCalc ? window.TaxCalc.calculateAnnualPAYE(taxableAnnual, age, members) : taxableAnnual * 0.25;
    const paye = Math.round((payeAnnual / freqMult) * 100) / 100;

    // ── UIF & SDL (via TaxCalc) ──────────────────────────────────────────
    const uif = window.TaxCalc ? window.TaxCalc.calculateUIF(uifRemuneration) : Math.min(uifRemuneration * 0.01, 177.12); // UIF ceiling R17,712/mo
    const sdl = window.TaxCalc ? window.TaxCalc.calculateSDL(sdlRemuneration) : sdlRemuneration * 0.01;
    ...
    return {
      basic, gross, bonus, otPay, phPay, paye, uif, sdl,
