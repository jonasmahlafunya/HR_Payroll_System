const Companies = {
    ...
    return `
        <div class="card" style="padding: 12px;">
          <div class="card-header" style="padding: 0 0 8px 0; border-bottom: 1px solid var(--gray-100); margin-bottom: 8px;">
            ...
            <span class="badge badge-success" style="font-size: 0.6rem; padding: 2px 6px;">
              ${company.status}
            </span>
          </div>
          ...
          <div class="card-body" style="padding: 0;">
            ...
            <div style="background: var(--gray-50); padding: 8px; border-radius: 4px; font-size: 0.7rem;">
               <div style="margin-bottom: 2px;">
                 <i class="fas fa-envelope text-muted"></i> ${company.email || 'No Email'}
               </div>
            ...
            <div style="display: flex; gap: 6px; margin-top: 10px;">
              <button class="btn btn-primary btn-sm" style="flex: 1.4; font-size: 0.7rem; padding: 5px;"
                onclick="Companies.viewCompanyDetail(${company.id})">
  ...
  // ─── Company Detail View (with Employees Tab) ─────────────────────────────────
  viewCompanyDetail: function (companyId) {
    ...
    const html = `
      <div class="card" style="width:100%;max-width:980px;margin:auto;max-height:92vh;overflow-y:auto;">
        ...
        <div class="tabs" style="padding:0 20px;margin-bottom:0;border-bottom:1px solid var(--gray-200);">
          ...
          <button class="tab-btn" id="cdTab_employees"
            ...
            <span class="badge badge-primary" style="margin-left:4px;font-size:0.65rem;padding:2px 6px;">
              ${window.DB.employees.filter(e => e.companyName === company.name || e.companyId == company.id).length}
            </span>
          </button>
        ...
        <div id="companyDetailContent" style="padding:20px;">
          ${this.renderDetailOverview(company)}
        </div>
      </div>
    `;
    ...
    return `
      ...
      <div style="display:flex;gap:10px;margin-bottom:16px;flex-wrap:wrap;">
        ...
        ${Object.entries(deptCounts).map(([dept, count]) =>
          `<div class="badge badge-gray" style="padding:5px 12px;font-size:0.78rem;">
             ${dept}: ${count}
           </div>`
        ).join('')}
      </div>
            ...
            <tbody>
              ${emps.map(emp => `
                <tr>
                  ...
                  <td>
                    <span class="badge badge-${emp.status === 'Active' ? 'success' : emp.status === 'Terminated' ? 'danger' : 'warning'}">
                      ${emp.status || 'Active'}
                    </span>
                  </td>
                  <td>
      ...
      steps: [
        {
          ...
          validate: () => {
            ...
            if (!name || !reg) {
              window.showAlert('Incomplete Details', 'Please fill in all required fields marked with (*).');
              return false;
            }
            if (!new RegExp("^\\d{4}/\\d{6}/\\d{2}$").test(reg)) {
          ...
          template: `
             <div class="grid-2">
               <div class="form-group"><label class="form-label">Tax Reference *</label>
                 <input type="text" id="comp_tax" class="form-control" required></div>
               <div class="form-group"><label class="form-label">VAT Number</label>
                 <input type="text" id="comp_vat" class="form-control"></div>
               <div class="form-group"><label class="form-label">UIF Number</label>
                 <input type="text" id="comp_uif" class="form-control"></div>
               <div class="form-group"><label class="form-label">SDL Number</label>
                 <input type="text" id="comp_sdl" class="form-control"></div>
             </div>
          `,
          validate: () => {
            if (!document.getElementById('comp_tax').value) {
              window.showAlert('Missing Tax Reference', 'The Tax Reference number is required.');
              return false;
            }
            return true;
          }
          ...
          template: `
             <div class="grid-2">
                <div class="form-group"><label class="form-label">Contact Person *</label>
                  <input type="text" id="comp_contact_person" class="form-control" required></div>
                <div class="form-group"><label class="form-label">Email Address *</label>
                  <input type="email" id="comp_email" class="form-control" required></div>
                <div class="form-group"><label class="form-label">Phone *</label>
                  <input type="text" id="comp_phone" class="form-control" required></div>
                <div class="form-group"><label class="form-label">Address</label>
                  <input type="text" id="comp_address" class="form-control"></div>
             </div>
          `,
          validate: () => {
            ...
            if (!person || !email || !phone) {
              window.showAlert('Missing Contact Info', 'Please fill in Contact Person, Email, and Phone.');
              return false;
            }
            return true;
          }
    ...
    const html = `
      <div class="card" style="width: 100%; max-width: 500px; margin: auto;">
         <div class="card-header">
            ...
            <button class="btn btn-outline btn-sm" onclick="closeModal('editCompanyModal')">
              <i class="fas fa-times"></i>
            </button>
         </div>
         <div class="card-body">
            ...
            <div class="grid-2">
               <div class="form-group">
                 ...
                 <input type="text" id="edit_comp_reg" class="form-control" value="${comp.registrationNumber || ''}">
               </div>
               <div class="form-group">
                 <label class="form-label">Tax Number</label>
                 <input type="text" id="edit_comp_tax" class="form-control" value="${comp.taxNumber || comp.taxReference || ''}">
               </div>
               <div class="form-group">
                 <label class="form-label">VAT Number</label>
                 <input type="text" id="edit_comp_vat" class="form-control" value="${comp.vatNumber || ''}">
               </div>
               <div class="form-group">
                 <label class="form-label">UIF Number</label>
                 <input type="text" id="edit_comp_uif" class="form-control" value="${comp.uifNumber || ''}">
               </div>
            </div>
            <div class="form-group">
              <label class="form-label">Address</label>
              <input type="text" id="edit_comp_address" class="form-control" value="${comp.address || ''}">
            </div>
  ...
  deleteCompany: function (id) {
    window.showConfirmation('Delete Company?', 'Are you sure? This will remove the company. (Soft Delete)', () => {
