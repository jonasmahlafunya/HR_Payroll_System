const Companies = {
  ...
  _togglePayDay: function(freq) {
    const group = document.getElementById('payDayGroup');
        ...
        <div class="card-body">
          ...
          <div class="grid-2">
            <div class="form-group"><label class="form-label">Pay Frequency</label>
              <select id="edit_comp_freq" class="form-control">
                ${['Monthly','Bi-Weekly','Weekly','Fortnightly'].map(f =>
                  `<option value="${f}" ${comp.payFrequency===f?'selected':''}>${f}</option>`).join('')}
              </select></div>
            <div class="form-group"><label class="form-label">Pay Day</label>
