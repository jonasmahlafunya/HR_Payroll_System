  ...
  render: function (container) {
    ...
    const html = `
      ...
      <div class="card" style="margin-bottom: 24px;">
        <div class="card-header"><h4 class="card-title">Team Leave Calendar</h4></div>
        <div class="card-body">
          <div style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; border: 1px solid var(--gray-200); border-radius: 8px; overflow: hidden;">
            ${['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => `<div style="padding: 10px; background: var(--gray-50); text-align: center; font-weight: 600; font-size: 0.8rem;">${d}</div>`).join('')}
            ${Array.from({ length: 31 }, (_, i) => {
              const day = i + 1;
              const hasLeave = day % 7 === 3 || day === 15;
              return `
                <div style="padding: 10px; min-height: 60px; border: 1px solid var(--gray-100); background: ${hasLeave ? 'rgba(99, 102, 241, 0.05)' : 'white'};">
                  <div style="font-size: 0.75rem; color: var(--gray-500);">${day}</div>
                  ${hasLeave ? `<div class="badge badge-primary" style="font-size: 0.6rem; padding: 2px 4px; margin-top: 4px; white-space: nowrap; overflow: hidden;">${day === 15 ? 'Thabo (Sick)' : 'Sarah (Annual)'}</div>` : ''}
                </div>
              `;
            }).join('')}
          </div>
