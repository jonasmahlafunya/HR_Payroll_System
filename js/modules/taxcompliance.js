...
const TaxCompliance = {
  // SARS Tax Tables 2025/2026 (1 Mar 2025 - 28 Feb 2026)
  taxTables: [
  ...
  medicalCredits: { mainMember: 364, firstDependent: 364, additionalDependent: 246 },
  limits: {
    uifCeiling: 17712,
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div style="display:flex; gap:8px; align-items:center;">
            <select class="form-control" id="irp5Company" style="width:200px; padding:6px 10px; font-size:0.82rem;">
    ...
    return `
      <div class="card">
        <div class="card-header">
          ...
          <div style="display:flex; gap:8px; align-items:center;">
            <select class="form-control" id="emp201Period" style="width:180px; padding:6px 10px; font-size:0.82rem;">
        ...
        <div class="card-body">
          ...
          <script>document.getElementById('emp201Period')?.addEventListener('change', function() { TaxCompliance.renderEMP201Preview(this.value); });<\/script>
        </div>
      </div>
    `;
  },
  ...
  renderUI19Tab: function () {
    ...
    return `
      <div class="card">
        <div class="card-header">
          <h4><i class="fas fa-user-shield"></i> UI-19 — Monthly UIF Declaration</h4>
          <div style="display:flex; gap:8px;">
            <select class="form-control" id="ui19Period" style="width:180px; padding:6px 10px; font-size:0.82rem;">
              <option value="">Select Period</option>${periodOptions}
            </select>
            <button class="btn btn-primary btn-sm" onclick="TaxCompliance.exportUI19()">
              <i class="fas fa-download"></i> Export UI-19 CSV
            </button>
          </div>
        </div>
        <div class="card-body">
  ...
  calculatePAYE: function (monthlyRemuneration, age = 35, medicalMembers = 0) {
    ...
    netTax = Math.max(0, netTax - (monthlyMTC * 12));
    return netTax / 12;
  },
  ...
  calculateBonusPAYE: function (basicSalary, bonusAmount, age = 35, medicalMembers = 0) {
    const regularMonthlyPAYE = this.calculatePAYE(basicSalary, age, medicalMembers);
    const regularAnnualPAYE  = regularMonthlyPAYE * 12;
    const annualWithBonus     = (basicSalary * 12) + bonusAmount;
    // Tax on combined income
    let grossTaxWithBonus = 0;
    for (let i = 0; i < this.taxTables.length; i++) {
      const bracket = this.taxTables[i];
      const prevLimit = i === 0 ? 0 : this.taxTables[i - 1].limit;
      if (annualWithBonus <= bracket.limit) {
        grossTaxWithBonus = bracket.deduction + ((annualWithBonus - prevLimit) * bracket.rate);
        break;
      }
      if (i === this.taxTables.length - 1) {
        grossTaxWithBonus = bracket.deduction + ((annualWithBonus - prevLimit) * bracket.rate);
      }
    }
    let rebate = this.rebates.primary;
    if (age >= 65) rebate += this.rebates.secondary;
    if (age >= 75) rebate += this.rebates.tertiary;
    const netTaxWithBonus = Math.max(0, grossTaxWithBonus - rebate);
