...
const Contracts = {
        ...
        <table>
          ...
          <tbody>
            ${filtered.map(c => {
              ...
              const statusColor = { Active:'success', Draft:'gray', Expired:'danger', 'Pending Signature':'warning' };
              return `
                <tr>
                  <td>
  ...
  generateContract: function () {
    ...
    // Automatically email to company HR email
    if (compEmail) {
      this._emailContract(contract, emp, company, contractHTML);
    }
...
<body>
  ...
  sendByEmail: function (contractId) {
    ...
    this._emailContract(c, emp, company, c.htmlContent);
  },

  _emailContract: function (contract, emp, company, html) {
    if (!html) { window.Toast.show('No contract HTML to send.', 'warning'); return; }

    const compEmail = company?.email;
    const empEmail  = emp?.email;

    if (!compEmail && !empEmail) {
      window.Toast.show('No email address found. Please add a company or employee email.', 'warning');
      return;
    }

    const recipient = compEmail || empEmail;
...
<table width="600" cellpadding="0" cellspacing="0"
  ...
  <tr><td style="padding:24px 32px;">
    <p style="font-size:15px;color:#1e293b;">Dear HR Team,</p>
    <p style="font-size:14px;color:#475569;margin:12px 0;">
      A new employment contract has been generated for the following employee:
    </p>
    ...
    <p style="font-size:14px;color:#475569;">
      ...
      Please review, print, and obtain signatures from both parties.
    </p>
  </td></tr>
  <tr><td style="background:#f8fafc;padding:14px 32px;text-align:center;
    ...
    fetch('api/email.php', {
      ...
      body: JSON.stringify({
        to:          recipient,
        subject:     `Employment Contract — ${empName} | ${compName}`,
        body_html:   emailBody,
