...
const PayrollWizard = {
      ...
      <div class="payroll-wizard">
        ...
        <div class="wizard-content card" style="margin-top: 24px;">
          ...
          <div class="card-footer" style="display: flex; justify-content: space-between; padding: 16px;">
            <button type="button" class="btn btn-outline" onclick="PayrollWizard.previousStep()" ${this.currentStep === 1 ? 'disabled' : ''}>
            ...
            <div style="display: flex; gap: 8px;">
              ...
              <button type="button" class="btn btn-primary" onclick="PayrollWizard.nextStep()" id="wizardNextBtn">
    ...
    renderStepCompanySelection: function () {
      ...
      <div style="max-width: 600px; margin: 0 auto;">
        ...
        <div class="form-group">
          ...
          <select class="form-control" id="wizard_payroll_type" onchange="PayrollWizard.payrollData.type = this.value; PayrollWizard.updateTypeNotice(this.value)">
        ...
        <div id="companyEmployeeNotice" class="alert alert-info" style="margin-top: 16px; display: ${this.payrollData.companyId ? 'block' : 'none'};">
          <i class="fas fa-info-circle"></i>
          <strong>${this.payrollData.companyName || ''}</strong> has ${this.payrollData.companyId ? this.getEmployeeCount() : 0} active employees.
        </div>
      </div>
    `;
    },
    ...
    updateTypeNotice: function (type) {
        const notices = {
            bonus: `<div class="alert alert-warning" style="margin-bottom:12px;"><i class="fas fa-info-circle"></i> <strong>13th Cheque / Bonus:</strong> PAYE will be calculated using the <em>directive (annual equivalent) method</em>. UIF and SDL do not apply to pure bonus payments. Each employee's bonus amount defaults to 1x basic salary.</div>`,
            retro: `<div class="alert alert-info" style="margin-bottom:12px;"><i class="fas fa-history"></i> <strong>Retro Pay:</strong> Enter the back-pay amount per employee in the "Retro Pay" column in Step 2. PAYE will be calculated on the difference only.</div>`,
            termination: `<div class="alert alert-danger" style="margin-bottom:12px;"><i class="fas fa-sign-out-alt"></i> <strong>Termination Payroll:</strong> Includes pro-rata salary, outstanding leave payout, and notice pay. Set up via the Employee Termination workflow first.</div>`,
        };
    ...
    renderStepCollectInputs: function () {
      ...
      <div>
                ...
                <tr>
                  ...
                  <td class="text-right">
                    <input type="number" class="form-control" style="width: 120px; text-align: right;"
                           value="${defaultBonus}" id="bonus_${emp.id}" placeholder="0.00">
                  </td>
                  <td class="text-right">
                    <input type="number" class="form-control" style="width: 120px; text-align: right;"
                  ...
                  <td class="text-right">
                    <input type="number" class="form-control" style="width: 100px; text-align: right;"
                           value="${totalGarnishee}" id="garnishee_${emp.id}" placeholder="0.00"
                           title="Garnishee / maintenance order deduction">
                  </td>
                  <td>
        ...
        ${isBonus ? '<div class="alert alert-warning" style="margin-top:12px;"><i class="fas fa-info-circle"></i> Bonus amounts default to 1× basic salary (13th cheque). Adjust as needed. PAYE will use the annual equivalent (directive) method.</div>' : ''}
        ${isRetro ? '<div class="alert alert-info" style="margin-top:12px;"><i class="fas fa-history"></i> Enter the back-pay amount per employee. PAYE will be calculated on the retro amount only using the marginal rate method.</div>' : ''}
      </div>
    `;
    },
    ...
    renderStepCalculation: function () {
        ...
        const calculations = employees.map(emp => {
            ...
            const age = (emp.dateOfBirth && window.TaxCalc)
                ? window.TaxCalc.calculateAge(emp.dateOfBirth)
                : 30;
            ...
            if (isBonus) {
                // Bonus run: gross = bonus amount only, no UIF/SDL on pure bonus
                gross = bonus;
                paye = window.TaxCalc
                    ? window.TaxCalc.calculateBonusPAYE(basic, bonus, age, medicalMembers)
                    : gross * 0.25;
                uif = 0;  // No UIF on bonus
                sdl = 0;  // No SDL on bonus
                totalDeductions = paye + deduction + garnishee;
                net = gross - totalDeductions;
            } else if (isRetro) {
                // Retro: bonus field = retro amount. PAYE on retro = (marginal rate × retro amount)
                gross = basic + allowances + bonus;
                const taxableIncome = gross - pensionContrib;
                const regularPAYE = window.TaxCalc
                    ? window.TaxCalc.calculatePAYE(basic + allowances - pensionContrib, age, medicalMembers)
                    : (basic + allowances) * 0.25;
                const retroPAYE = window.TaxCalc
                    ? window.TaxCalc.calculateBonusPAYE(basic + allowances, bonus, age, medicalMembers) / 12
                    : bonus * 0.25;
                paye = regularPAYE + retroPAYE;
                ...
                const taxableIncome = gross - pensionContrib;
                paye = window.TaxCalc
                    ? window.TaxCalc.calculatePAYE(taxableIncome, age, medicalMembers)
                    : gross * 0.25;
                uif = Math.min(gross * 0.01, 177.12);
                sdl = gross * 0.01;
                totalDeductions = paye + uif + deduction + garnishee + pensionContrib;
      ...
      <div style="max-width: 600px; margin: 0 auto;">
        ...
        <div class="form-group">
          <label class="form-label">
            <input type="checkbox" id="confirm_finalize" style="margin-right: 8px;" onchange="document.getElementById('finalizeBtn').disabled = !this.checked;">
    ...
    selectCompany: function (companyId) {
        ...
        if (company) {
            ...
            const noticeBox = document.getElementById('companyEmployeeNotice');
            if (noticeBox) {
                noticeBox.innerHTML = `<i class="fas fa-info-circle"></i> <strong>${company.name}</strong> has ${this.getEmployeeCount()} active employees.`;
