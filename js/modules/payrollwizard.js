...
const PayrollWizard = {
  ...
  start: function () {
    ...
    this.payrollData = {
      ...
      type: 'regular', frequency: 'Monthly', employees: [], errors: [], warnings: [],
      variances: [], calculations: [], approvalToken: null
    };
    ...
    return `
      <div style="max-width:560px;">
        ...
        <div class="form-group">
          ...
          <select class="form-control" id="wizard_type" onchange="PayrollWizard.payrollData.type=this.value; document.getElementById('wizard_frequency').value=this.value; PayrollWizard.payrollData.frequency=this.value;">
            <option value="Monthly" ${this.payrollData.type === 'Monthly' ? 'selected' : ''}>Monthly Payroll</option>
            <option value="Weekly" ${this.payrollData.type === 'Weekly' ? 'selected' : ''}>Weekly Payroll</option>
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label class="form-label">Payroll Frequency <span style="color:var(--danger)">*</span></label>
          <select class="form-control" id="wizard_frequency" onchange="PayrollWizard.payrollData.frequency=this.value;PayrollWizard._updateFreqInfo()">
            <option value="Monthly" ${this.payrollData.frequency === 'Monthly' ? 'selected' : ''}>Monthly</option>
            <option value="Bi-Weekly" ${this.payrollData.frequency === 'Bi-Weekly' ? 'selected' : ''}>Bi-Weekly (Fortnightly)</option>
            <option value="Weekly" ${this.payrollData.frequency === 'Weekly' ? 'selected' : ''}>Weekly</option>
            <option value="All" ${this.payrollData.frequency === 'All' ? 'selected' : ''}>All Frequencies (Mixed Run)</option>
          </select>
          <div class="form-hint">Employees with a different pay frequency will be prorated automatically.</div>
        </div>
        <div class="form-group">
        ...
        <div id="companyInfo" class="alert alert-info"
          ...
          ${this.payrollData.companyId ? this.getEmployeeCount() : 0} active employees match this frequency
        </div>
        <div id="freqInfo" style="display:${this.payrollData.companyId ? 'block' : 'none'};"></div>
      </div>`;
  },

  _updateFreqInfo: function () {
    const info = document.getElementById('companyInfo');
    const emps = this.getCompanyEmployees();
    if (info) {
      info.innerHTML = '<i class="fas fa-users"></i> <strong>' + (this.payrollData.companyName || '') + '</strong> — ' + emps.length + ' active employees match this frequency';
    }
  },
  ...
  renderStepInputs: function () {
    ...
    const isWeekly = this.payrollData.frequency === 'Weekly' || this.payrollData.frequency === 'Bi-Weekly';

    return `
      <p style="color:var(--gray-600);margin-bottom:16px;">
        Review and add any hours, bonuses, or deductions for this period.
      </p>
      <div class="table-responsive">
        <table>
          <thead>
            <tr>
              <th>Employee</th>
              <th class="text-right">Rate/Basic</th>
              <th class="text-right">Normal Hours</th>
              ${isWeekly ? `
                <th class="text-right">Overtime (1.5x)</th>
                <th class="text-right">Public Hol. (2x)</th>
                <th class="text-right">Other Add.</th>
              ` : `
          ...
          <tbody>
            ${emps.map(emp => {
      const defaultHours = isWeekly ? 40 : 0;
      return `
              <tr>
                <td>
                ...
                <td class="text-right">${window.formatCurrency(emp.ratePerHour > 0 ? emp.ratePerHour : emp.basicSalary || 0)}${emp.ratePerHour > 0 ? '/hr' : ''}</td>
                <td class="text-right">
                  <input type="number" class="form-control" style="width:80px;text-align:right;"
                    value="${defaultHours}" id="hours_${emp.id}">
                </td>
                ${isWeekly ? `
                <td class="text-right">
                  <input type="number" class="form-control" style="width:90px;text-align:right;"
                    value="0" id="ot_${emp.id}">
                </td>
                <td class="text-right">
                  <input type="number" class="form-control" style="width:90px;text-align:right;"
                    value="0" id="ph_${emp.id}">
                </td>
                <td class="text-right">
                  <input type="number" class="form-control" style="width:90px;text-align:right;"
                    value="0" id="other_${emp.id}">
                </td>
                ` : `
                <td class="text-right">
  ...
  renderStepCalculation: function () {
    const emps = this.getCompanyEmployees();
    const isBonus = this.payrollData.type === 'bonus';
    const freq = this.payrollData.frequency || 'Monthly';

    const calcs = emps.map(emp => {
      const isWeeklyComp = freq === 'Weekly' || freq === 'Bi-Weekly';
      const isMonthly = freq === 'Monthly';
      const input = (this.wizardData && this.wizardData.inputs && this.wizardData.inputs[emp.id]) || {};

      let bonusInput, deductionInput, garnisheeInput, hoursWorked, otHours, phHours;

      if (isWeeklyComp) {
        hoursWorked = parseFloat(document.getElementById(`hours_${emp.id}`)?.value || (isMonthly ? 0 : 40));
        otHours = parseFloat(document.getElementById(`ot_${emp.id}`)?.value || 0);
        phHours = parseFloat(document.getElementById(`ph_${emp.id}`)?.value || 0);
        bonusInput = parseFloat(document.getElementById(`other_${emp.id}`)?.value || 0);
        deductionInput = 0;
        garnisheeInput = 0;
      } else {
        bonusInput = parseFloat(document.getElementById(`bonus_${emp.id}`)?.value || 0);
        deductionInput = parseFloat(document.getElementById(`deduction_${emp.id}`)?.value || 0);
        garnisheeInput = parseFloat(document.getElementById(`garnishee_${emp.id}`)?.value || 0);
        hoursWorked = 0;
        otHours = 0;
        phHours = 0;
      }

      const rate = Number(emp.ratePerHour || 0);
      const otPay = Math.round(otHours * rate * 1.5 * 100) / 100;
      const phPay = Math.round(phHours * rate * 2.0 * 100) / 100;

      // Use the unified calculator
      const calcResult = window.Payroll.calculateTax({
        ...emp,
        bonus: bonusInput,
        garnishee: garnisheeInput
      }, hoursWorked, { otPay, phPay });
      ...
      return {
        ...
        uif: calcResult.uif,
        sdl: calcResult.sdl,
        pension: calcResult.pension,
        medical: calcResult.medical,
        otherDeductions: calcResult.deductionSum + deductionInput,
        garnishee: calcResult.garnishee,
        net: calcResult.net,
        customBenefits: calcResult.customBenefits
      };
    });
  ...
  sendForApproval: async function () {
    ...
    const newRun = {
      ...
      frequency: this.payrollData.frequency || 'Monthly',
    ...
    // Also add to Documents tab
    window.DB.documents = window.DB.documents || [];
    calcs.forEach(calc => {
      const empId = calc.employeeId || calc.id;
      const emp = (window.DB.employees || []).find(e => e.id == empId);
      const empName = emp ? `${emp.firstName} ${emp.lastName}` : calc.employeeName || 'Employee';
      window.DB.documents.push({
        id: 'DOC_' + runId + '_' + empId,
        name: `Payslip — ${empName} (${this.payrollData.period})`,
        category: 'Payslip',
        employeeId: empId,
        companyWide: false,
        description: `${this.payrollData.period} payslip for ${empName}`,
        size: 0, mimeType: 'application/json',
        fileName: `Payslip_${empId}_${this.payrollData.period.replace(/\s/g, '_')}.pdf`,
        dataUrl: null,
        uploadedAt: new Date().toISOString(),
        uploadedBy: window.currentUser?.name || 'System',
        sourceId: runId, sourceType: 'payslip'
      });
    });
  ...
  getCompanyEmployees: function () {
    ...
    return (window.DB.employees || []).filter(e => {
      ...
      // Company match
      const compMatch = (e.companyId && e.companyId == cid)
        || (e.companyName && e.companyName === cname)
        || (!e.companyName && !e.companyId && cid == (window.DB.companies[0]?.id));
      if (!compMatch) return false;
      // Frequency match ("All" = mixed run, include everyone)
      if (freq === 'All') return true;
      const empFreq = e.payFrequency || 'Monthly';
      return empFreq === freq;
    });
  }
};
