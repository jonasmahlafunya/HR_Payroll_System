    ...
    container.innerHTML = `
      ...
      <div class="grid-4" style="gap:12px;margin-bottom:16px;">
        ${[
          ...
          ['Policies',        docs.filter(d=>d.category==='Policy').length,   'fas fa-gavel', 'warning'],
    ...
    if (doc.dataUrl) {
      ...
      if (win) {
        if (doc.mimeType?.includes('image')) {
          win.document.write(`<html><body style="margin:0;background:#1e293b;display:flex;justify-content:center;align-items:center;min-height:100vh;">
            <img src="${doc.dataUrl}" style="max-width:98vw;max-height:98vh;object-fit:contain;">
          </body></html>`);
          ...
          win.document.write(`<html><body>
            ...
            <p>Size: ${this._formatSize(doc.size)}</p>
            <a href="${doc.dataUrl}" download="${doc.fileName}">Download</a>
          </body></html>`);
        }
      }
    } else {
      // No file data — show info modal
      window.showAlert('Document Info', `
        ...
        Size: ${this._formatSize(doc.size)}<br>
        <em>File content not available — document was saved without a file.</em>
      `);
    }
  },
