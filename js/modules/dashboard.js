...
const Dashboard = {
  ...
  render: function (container) {
    ...
    container.innerHTML = `
      ...
      <!-- New KPI Insight Widgets -->
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px;">

        <!-- Leave Balance Summary -->
        <div class="card" style="padding:16px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
            <div style="width:32px;height:32px;border-radius:8px;background:var(--success-soft);
                        display:flex;align-items:center;justify-content:center;color:var(--success);">
              <i class="fas fa-umbrella-beach" style="font-size:0.85rem;"></i>
            </div>
            <div style="font-size:0.78rem;font-weight:700;color:var(--gray-700);">Leave Balances</div>
          </div>
          ${(() => {
        const totalAnnual = activeEmps.reduce((s, e) => s + (e.leaveBalances?.annual || 0), 0);
        const totalSick = activeEmps.reduce((s, e) => s + (e.leaveBalances?.sick || 0), 0);
        const zeroBalance = activeEmps.filter(e => (e.leaveBalances?.annual || 0) <= 0).length;
        return '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">' +
          '<div style="background:var(--gray-50);border-radius:6px;padding:8px;text-align:center;">' +
          '<div style="font-size:0.62rem;color:var(--gray-500);text-transform:uppercase;">Annual</div>' +
          '<div style="font-size:1rem;font-weight:800;color:var(--success);">' + totalAnnual.toFixed(1) + '</div>' +
          '<div style="font-size:0.58rem;color:var(--gray-400);">days total</div>' +
          '</div>' +
          '<div style="background:var(--gray-50);border-radius:6px;padding:8px;text-align:center;">' +
          '<div style="font-size:0.62rem;color:var(--gray-500);text-transform:uppercase;">Sick</div>' +
          '<div style="font-size:1rem;font-weight:800;color:var(--info);">' + totalSick.toFixed(1) + '</div>' +
          '<div style="font-size:0.58rem;color:var(--gray-400);">days total</div>' +
          '</div>' +
          '</div>' +
          (zeroBalance > 0 ? '<div style="margin-top:8px;font-size:0.7rem;color:var(--danger);"><i class="fas fa-exclamation-triangle"></i> ' + zeroBalance + ' employee' + (zeroBalance > 1 ? 's' : '') + ' at 0 annual leave</div>' : '');
      })()}
        </div>
        ...
        <div class="card" style="padding:16px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
            <div style="width:32px;height:32px;border-radius:8px;background:var(--primary-soft);
                        display:flex;align-items:center;justify-content:center;color:var(--primary);">
              <i class="fas fa-clock" style="font-size:0.85rem;"></i>
            </div>
            <div style="font-size:0.78rem;font-weight:700;color:var(--gray-700);">Next Payroll</div>
          </div>
          ${(() => {
        const payDay = parseInt(window.DB.settings?.payDay) || 25;
        ...
        const nextLabel = next.toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' });
        const urgency = diff <= 3 ? 'danger' : diff <= 7 ? 'warning' : 'primary';
        return '<div style="text-align:center;padding:4px 0;">' +
          '<div style="font-size:2.2rem;font-weight:900;color:var(--' + urgency + ');line-height:1;">' + diff + '</div>' +
          '<div style="font-size:0.72rem;color:var(--gray-500);margin-top:2px;">days until ' + nextLabel + '</div>' +
          '<div style="margin-top:8px;height:4px;background:var(--gray-100);border-radius:99px;overflow:hidden;">' +
          '<div style="height:100%;width:' + Math.max(5, 100 - (diff / 30 * 100)) + '%;background:var(--' + urgency + ');border-radius:99px;transition:width 0.5s;"></div>' +
          '</div>' +
          '</div>';
      })()}
        </div>
        ...
        <div class="card" style="padding:16px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
            <div style="width:32px;height:32px;border-radius:8px;background:var(--warning-soft);
                        display:flex;align-items:center;justify-content:center;color:var(--warning);">
              <i class="fas fa-hourglass-half" style="font-size:0.85rem;"></i>
            </div>
            <div style="font-size:0.78rem;font-weight:700;color:var(--gray-700);">Probation Ending</div>
          </div>
          ${(() => {
        const probMonths = parseInt(window.DB.settings?.probationMonths) || 3;
        const soon = activeEmps.filter(e => {
          if (!e.startDate) return false;
          const probEnd = new Date(e.startDate);
          probEnd.setMonth(probEnd.getMonth() + probMonths);
          const diff = Math.ceil((probEnd - now) / 86400000);
          return diff > 0 && diff <= 30;
        }).map(e => {
          const probEnd = new Date(e.startDate);
          probEnd.setMonth(probEnd.getMonth() + probMonths);
          const diff = Math.ceil((probEnd - now) / 86400000);
          return { name: e.firstName + ' ' + e.lastName, days: diff };
        }).sort((a, b) => a.days - b.days);
        if (!soon.length) return '<div style="text-align:center;padding:12px 0;color:var(--gray-400);font-size:0.8rem;"><i class="fas fa-check-circle" style="color:var(--success);margin-right:4px;"></i>No probations ending soon</div>';
        return soon.slice(0, 3).map(s =>
          ...
          '<span style="font-size:0.75rem;font-weight:500;color:var(--gray-700);">' + s.name + '</span>' +
          '<span class="badge badge-' + (s.days <= 7 ? 'danger' : 'warning') + '" style="font-size:0.6rem;">' + s.days + 'd</span>' +
          '</div>'
        ).join('') + (soon.length > 3 ? '<div style="font-size:0.68rem;color:var(--primary);margin-top:4px;">+' + (soon.length - 3) + ' more</div>' : '');
      })()}
        </div>
        ...
        <div class="card" style="padding:16px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
            <div style="width:32px;height:32px;border-radius:8px;background:var(--danger-soft);
                        display:flex;align-items:center;justify-content:center;color:var(--danger);">
              <i class="fas fa-file-exclamation fa-file-alt" style="font-size:0.85rem;"></i>
            </div>
            <div style="font-size:0.78rem;font-weight:700;color:var(--gray-700);">Expiring Documents</div>
          </div>
          ${(() => {
        const allDocs = [...(documents || []), ...(contracts || [])];
        const expiring = allDocs.filter(d => {
          ...
          const exp = d.expiryDate || d.endDate;
          const diff = Math.ceil((new Date(exp) - now) / 86400000);
          return { name: d.name || d.title || 'Document', days: diff, cat: d.category || 'Contract' };
        }).sort((a, b) => a.days - b.days);
        const critical = expiring.filter(d => d.days <= 30).length;
        const warn = expiring.filter(d => d.days > 30 && d.days <= 60).length;
        const notice = expiring.filter(d => d.days > 60).length;
        if (!expiring.length) return '<div style="text-align:center;padding:12px 0;color:var(--gray-400);font-size:0.8rem;"><i class="fas fa-check-circle" style="color:var(--success);margin-right:4px;"></i>No documents expiring soon</div>';
        return '<div style="display:flex;gap:6px;margin-bottom:8px;">' +
          (critical ? '<div style="flex:1;text-align:center;padding:6px;background:var(--danger-soft);border-radius:6px;"><div style="font-size:1rem;font-weight:800;color:var(--danger);">' + critical + '</div><div style="font-size:0.58rem;color:var(--danger);">≤30 days</div></div>' : '') +
          (warn ? '<div style="flex:1;text-align:center;padding:6px;background:var(--warning-soft);border-radius:6px;"><div style="font-size:1rem;font-weight:800;color:var(--warning);">' + warn + '</div><div style="font-size:0.58rem;color:var(--warning);">31-60 days</div></div>' : '') +
          (notice ? '<div style="flex:1;text-align:center;padding:6px;background:var(--info-soft);border-radius:6px;"><div style="font-size:1rem;font-weight:800;color:var(--info);">' + notice + '</div><div style="font-size:0.58rem;color:var(--info);">61-90 days</div></div>' : '') +
          '</div>' +
          expiring.slice(0, 2).map(d =>
            '<div style="font-size:0.7rem;color:var(--gray-600);padding:2px 0;">' +
            '<i class="fas fa-exclamation-circle" style="color:var(--' + (d.days <= 30 ? 'danger' : d.days <= 60 ? 'warning' : 'info') + ');margin-right:3px;"></i>' +
            d.name.substring(0, 25) + (d.name.length > 25 ? '…' : '') + ' — <strong>' + d.days + 'd</strong>' +
            '</div>'
          ).join('');
      })()}
        </div>
