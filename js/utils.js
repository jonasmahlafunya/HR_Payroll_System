...
class Toast {
  static show(message, type = 'info', duration = 5000) {
    // Only redirect explicit 'danger' type to alert popup for critical errors.
    // 'warning' type shows as a regular toast to avoid blocking innocuous messages.
    if (type === 'danger') {
      if (window.showAlert) {
...
// Confirmation Dialog (NexaPOS Style)
function showConfirmation(title, message, onConfirm, onCancel = null) {
  ...
  backdrop.innerHTML = `
    <div class="nexa-confirm-box">
      <div class="nexa-confirm-header">
        <h3>${title}</h3>
        <button class="btn btn-sm btn-outline" onclick="document.getElementById('${backdropId}').remove()" style="padding: 4px 8px; border-radius: 6px;">
          <i class="fas fa-times"></i>
        </button>
      </div>
  ...
  document.getElementById('nexaConfirmBtn').onclick = function () {
    backdrop.remove();
    if (onConfirm) onConfirm();
  };

  document.getElementById('nexaCancelBtn').onclick = function () {
    backdrop.remove();
...
// Alert Dialog (NexaPOS Centered Popup)
  ...
  backdrop.innerHTML = `
    <div class="nexa-alert-box">
      <div class="nexa-alert-icon-wrap">
        <div class="nexa-alert-icon ${t.cls}">
...
class Wizard {
  constructor(config) {
    if (window.currentWizard && window.currentWizard !== this) {
      console.warn("Another wizard is already active. Overwriting state may cause issues.");
    }
