...
class Auth {
  ...
  static logout(reason = '') {
    // Notify server to invalidate session cookie
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.valid && data.user) {
          // Re-sync with local DB for full permission profile
          const localUser = (window.DB.users || []).find(u => u.username === data.user.username);
  ...
  static async verify2FA(username, code) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function (e) {
      ...
      const twoFactorField = document.getElementById('twoFactor');
      const twoFactorSection = document.getElementById('twoFactorSection');
      const code = twoFactorField?.value.trim();
      ...
      try {
        ...
        if (twoFactorSection && twoFactorSection.style.display !== 'none') {
          // Verify 2FA
          user = await Auth.verify2FA(username, code);
        } else {
          // Initial Login
          ...
          if (result.status === '2fa_required') {
            ...
            if (twoFactorField) twoFactorField.focus();
            if (btnText) btnText.textContent = 'Verify Code';
            // Hide password field for cleaner UI during 2FA
        ...
        if (btnText) {
          ...
          // If we're in 2FA mode, ensure text is correct
          if (twoFactorSection && twoFactorSection.style.display !== 'none') {
  ...
  window.addEventListener('focus', function () {
    // Session is validated by httpOnly cookies now; refocusing is handled by bootstrap logic
  });
});
