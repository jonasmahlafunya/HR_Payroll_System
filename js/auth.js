...
class Auth {
  ...
  static hasPermission(page) {
    ...
    // Basic access
    if (['dashboard', 'profile'].includes(page)) return true;
    if (page === 'employeeportal') return p.includes('self.view');

    // Mappings to specific permissions
    if (['employees', 'orgchart', 'departments', 'companies'].includes(page)) return p.includes('employees.view') || p.includes('employees.edit');
    if (['contracts', 'recruitment', 'onboarding', 'offboarding', 'engagement', 'training', 'succession', 'disciplinary', 'assets'].includes(page)) return p.includes('employees.edit');
    ...
    if (['timeattendance', 'leave', 'shifts'].includes(page)) return p.includes('leave.manage') || p.includes('employees.view');
    if (['reports', 'analytics'].includes(page)) return p.includes('reports.view');
    if (['documents', 'lettergenerator'].includes(page)) return p.includes('documents.manage');
    if (page === 'settings') return p.includes('settings.admin');

    return p.includes(page);
  }
  ...
  static async verifySession() {
    try {
      ...
      if (data.valid && data.user) {
        ...
        // Critical: Since localStorage is disabled natively, we absolutely MUST fetch 
        // the state from MySQL before granting the authenticated dashboard view.
        if (typeof window.autoSync === 'function') {
          await window.autoSync();
        }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('loginForm')?.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      // Email send failure — surface the server's error detail to the user
