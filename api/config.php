...
// ── Database config ───────────────────────────────────────────
define('DB_HOST', env('DB_HOST', 'sql303.infinityfree.com'));
define('DB_USER', env('DB_USER', 'if0_41384788'));
define('DB_PASS', env('DB_PASS', ''));           // No default — must be in .env
define('DB_NAME', env('DB_NAME', 'if0_41384788_nexa_hrpayroll'));
...
set_error_handler(function(int $errno, string $errstr) {
    if (APP_ENV !== 'development') {
        json_response(['error' => 'An internal error occurred.'], 500);
    }
    return false;
});