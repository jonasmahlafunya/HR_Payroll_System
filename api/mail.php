...
<table width="640" cellpadding="0" cellspacing="0"
  ...
  <tr><td style="padding:20px 32px;">
    <p style="font-size:14px;color:#475569;line-height:1.7;margin:0;">
      Please review the breakdown above and click the <strong>Approve Payroll</strong> button
      to authorise processing. Once approved, the payroll officer will generate and submit
...
if (!empty($employees)) {
    $textBody .= "EMPLOYEE BREAKDOWN\n" . str_repeat('-', 40) . "\n";
    foreach ($employees as $emp) {
        $textBody .= sprintf("%-25s  Gross: R%-10s  Net: R%s\n",
            $emp['employeeName'] ?? '—',
            number_format((float)($emp['gross'] ?? 0), 2),
            number_format((float)($emp['net'] ?? 0), 2)
        );
    }
    $textBody .= "\n";
}
...
// Send
$result = send_gmail_smtp($to, $subject, $htmlBody, $textBody, $smtpPass);
...
// ─────────────────────────────────────────────────────────────────────────────
function send_gmail_smtp(string $to, string $subject, string $html, string $text, string $password): mixed {
    $host   = 'smtp.gmail.com';
    $port   = 587;
    $user   = GMAIL_USER;
    $from   = GMAIL_USER;
    ...
    $msg  = "Date: " . date('r') . "\r\n";
    $msg .= "From: {$encFrom} <{$from}>\r\n";
    $msg .= "To: {$to}\r\n";
    $msg .= "Reply-To: {$from}\r\n";
    $msg .= "Subject: {$encSubj}\r\n";
    $msg .= "MIME-Version: 1.0\r\n";
    $msg .= "Content-Type: multipart/alternative; boundary=\"{$boundary}\"\r\n";
    $msg .= "X-Mailer: NexaHR-Payroll\r\n\r\n";
